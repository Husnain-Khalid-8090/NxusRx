  ...
  const isActive = (value) => (location?.pathname === value ? 'active' : '')
    return (
        ...
        >
            ...
            {sideNavData && sideNavData.length > 0 ? sideNavData.map((item,index) => (
          <ListItem key={index} disablePadding className={isActive(`${item?.link_to}`)} onClick={()=>history(`${item?.link_to}`,{replace:true})}>
          <ListItemButton>
              <ListItemIcon>
                  <img src={item?.icon}/>
              </ListItemIcon>
     ...
     <ListItem disablePadding className={isActive('/profile')} onClick={()=>history('/dash/profile',{replace:true})}>
     <ListItemButton>
         <ListItemIcon>
             <img src={userProfile}/>
         </ListItemIcon>
